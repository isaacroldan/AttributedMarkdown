...
{
    NSAttributedString *attributedString = [MMMarkdown attributedStringWithMarkdown:markdown error:nil];
...
- (void)updateTextViewAfterAddingAllElements
{
    ...
    if ([self.currentOffset intValue] != self.frame.size.height) {
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, [self.currentOffset intValue]);
    }
...
{
    YLImageView *imageView = [[YLImageView alloc] initWithFrame:CGRectMake(5, [self.currentOffset intValue], self.frame.size.width-10, kImagePlaceholderHeight)];
    if (cachedImage) {
        self.subviewsToBeRendered = @(self.subviewsToBeRendered.intValue - 1);
...
- (void)addAttributedString:(NSMutableAttributedString *)attString
{
    ...
    CGSize size = [(NSMutableAttributedString*)attString boundingRectWithSize:CGSizeMake(self.frame.size.width-6.0, 999) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) context:NULL].size;
...
- (void)updateTextViewHeight
{
    if ([self.previuosHeightOffset intValue] != 0) {
        [self setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.frame.size.height-[self.previuosHeightOffset intValue])];
    }
    [self.gifDelegate textViewHeightWasUpdatedWithHeight:self.frame.size.height];
}
